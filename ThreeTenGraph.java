...
class ThreeTenGraph<V, E> implements Graph<V, E>, DirectedGraph<V, E> {
	...
	//
	/**
	 * Instance variable holding a map of the graph
	 */
	private HashMap<E, Pair<V>> link = new HashMap<E, Pair<V>>();
	/**
	 * Instance variable holding a collection of edges
	 */
	private Collection<E> edges = new HashSet<E>();
	/**
	 * Instance variable holding a collection of vertices
	 */
	...
	public Collection<E> getInEdges(V vertex) {
		...
		ArrayList<E> edges = new ArrayList<E>(getEdges());
		Collection<E> edgesList = new HashSet<E>();

		for (int i = 0; i < edges.size(); i++) {
			if (link.get(edges.get(i)).getSecond().equals(vertex)) {
	...
	public Collection<E> getOutEdges(V vertex) {
		...
		for (int i = 0; i < edges.size(); i++) {
			if (link.get(edges.get(i)).getFirst().equals(vertex)) {
				edgesList.add(edges.get(i));
			}
		}
		return edgesList;
	...
	public Collection<V> getPredecessors(V vertex) {
		...
		ArrayList<E> inEdges = new ArrayList<E>(getInEdges(vertex));
		Collection<V> predecessors = new HashSet<V>();

		for (int i = 0; i < inEdges.size(); i++) {
			predecessors.add(link.get(inEdges.get(i)).getFirst());
		}
		return predecessors;
	}
	...
	public Collection<V> getSuccessors(V vertex) {
		...
		ArrayList<E> outEdges = new ArrayList<E>(getOutEdges(vertex));
		Collection<V> successor = new HashSet<V>();

		for (int i = 0; i < outEdges.size(); i++) {
			successor.add(link.get(outEdges.get(i)).getSecond());
		}
		return successor;
	}
	...
	public boolean isPredecessor(V v1, V v2) {
		...
		if (getPredecessors(v2).contains(v1)) {
			return true;
		}
		return false;
	}
	...
	public boolean isSuccessor(V v1, V v2) {
		...
		if (getSuccessors(v2).contains(v1)) {
			return true;
		}
		return false;
	}
	...
	public V getSource(E directed_edge) {
		if (directed_edge == null || !containsEdge(directed_edge)) {
			return null;
		}
		if (getEdgeType(directed_edge).equals(EdgeType.DIRECTED) && containsEdge(directed_edge)) {// check
			return link.get(directed_edge).getFirst();
		}
		return null;
	}
	...
	public V getDest(E directed_edge) {
		...
		if (getEdgeType(directed_edge).equals(EdgeType.DIRECTED) && containsEdge(directed_edge)) {// check
			return link.get(directed_edge).getSecond();
		}
		return null;
	}
	...
	public Pair<V> getEndpoints(E edge) {
		...
		return new Pair<V>(link.get(edge).getFirst(), link.get(edge).getSecond());
	}
	...
	public Collection<V> getNeighbors(V vertex) {
		...
		ArrayList<V> successors = new ArrayList<V>(getSuccessors(vertex));
		ArrayList<V> predecessors = new ArrayList<V>(getPredecessors(vertex));
		Collection<V> neighbors = new HashSet<V>();

		for (int i = 0; i < successors.size(); i++) {
			neighbors.add(successors.get(i));
		}

		for (int i = 0; i < predecessors.size(); i++) {
			neighbors.add(predecessors.get(i));
		}
	...
	public Collection<E> getIncidentEdges(V vertex) {
		...
		for (int i = 0; i < inEdges.size(); i++) {
			edges.add(inEdges.get(i));
		}

		for (int i = 0; i < outEdges.size(); i++) {
			edges.add(outEdges.get(i));
		}
		return edges;
	}
	 ...
	 *         edge is not present
	 */
	public Collection<V> getIncidentVertices(E edge) {// ND
		if (edge == null || !containsEdge(edge)) {
			return null;
		}
		Collection<V> vertices = new HashSet<V>();
		vertices.add(getEndpoints(edge).getFirst());
		vertices.add(getEndpoints(edge).getSecond());
	 ...
	 * connected to v2 via a given <i>directed</i> edge e if v1 == e.getSource() v2
	 * == e.getDest() evaluates to true. (v1 and v2 are connected by an undirected
	 * edge u if u is incident to both v1 and v2.)
	 * 
	 * @param v1
	 *            vertex 1
	 * @param v2
	 ...
	 *         either vertex is not present) Hypergraph#findEdgeSet(Object, Object)
	 */
	public E findEdge(V v1, V v2) {// ND
		if (v1 == null || v2 == null) {
			return null;
		}

		ArrayList<E> edgeList = new ArrayList<E>(getEdges());

		for (int i = 0; i < edgeList.size(); i++) {
			if (link.get(edgeList.get(i)).getFirst().equals(v1)) {
				if (link.get(edgeList.get(i)).getSecond().equals(v2)) {
	...
	public boolean isIncident(V vertex, E edge) {
		...
		if (getIncidentEdges(vertex).contains(edge) && getIncidentVertices(edge).contains(vertex)) {
			return true;
		}
		return false;
	}
	 ...
	 * @return the degree of this node Hypergraph#getNeighborCount(Object)
	 */
	 ...
	 *             Hypergraph#addEdge(Object, Collection) #addEdge(Object, Object,
	 *             Object, EdgeType)
	 */
	public boolean addEdge(E e, V v1, V v2) {
		// checks if edge or vertices is null, or if a different vertex set in this
		// graph is already connected by edge, or if vertices are not a legal vertex set
		// for edge
		if (v1 == null || v2 == null || e == null) {
			throw new IllegalArgumentException("Error: A value you are adding is Null");
		}
		if (findEdge(v1,v2) != null) {
			throw new IllegalArgumentException("Error: Edge already exists");
		}
		link.put(e, new Pair<V>(v1, v2));
		edges.add(e);
		return true;
	}
	...
	public boolean addVertex(V vertex) {
		if (vertex == null) {
			throw new IllegalArgumentException("Error: Vertex was Null");
		}
	...
	public boolean removeVertex(V vertex) {
		...
		ArrayList<E> incidentEdges = new ArrayList<E>(getIncidentEdges(vertex));
		
		for (int i = 0; i < incidentEdges.size(); i++) {
			removeEdge(incidentEdges.get(i));
		}
	 ...
	 * <p>
	 * <b>Note</b>: for purposes of this method, v1 is only considered to be
	 * connected to v2 via a given <i>directed</i> edge d if v1 == d.getSource() v2
	 * == d.getDest() evaluates to true. (v1 and v2 are connected by an undirected
	 * edge u if u is incident to both v1 and v2.)
	 * 
	 * @param v1
	 *            vertex 1
	 ...
	 *         either vertex is not present Hypergraph#findEdge(Object, Object)
	 */
